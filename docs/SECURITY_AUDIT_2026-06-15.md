...
- **Coach can read/edit/promote any junior, not just their roster.** `GET/PUT /api/juniors/:id`, `/:id/progress`, `/monthly-report`, `POST /:id/promote`, `GET /api/evaluations(?junior_id=)`, `GET /api/attendance` all treat *any* coach as in-scope, while sibling endpoints (schedule, sessions, classes, handicap-journey, coach-sign) correctly enforce `coach_id == caller.id`. **This is a product decision:** if coaches are trusted club-wide readers, it's acceptable (only the cross-roster *promote*/*edit* mutations are concerning); if "coach sees only assigned juniors" is a hard rule (CLAUDE.md says it is), these need `coach_id` scoping server-side. *Note:* the frontend coach-scoping I added (`useCoachJuniors`) is **UX only, not a boundary** — `GET /api/juniors` still returns the whole roster to a coach token.
- **Coach can reassign sessions/classes** by setting `coach_id` via the unrestricted `update_session`/`update_class` setattr loops (`app/sessions/controllers.py`). **Fix:** allowlist editable fields; never accept `coach_id` from a non-admin.
...
2. **Next (HIGH):** hole-scores IDOR + `POST /rounds` ownership (score/handicap tampering by any player).
3. **Decision needed:** is a coach roster-scoped or a club-wide reader? Drives `/api/juniors`, `/api/evaluations`, promote, attendance scoping.
