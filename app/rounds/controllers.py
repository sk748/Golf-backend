...
def list_rounds(user_id=None, course_id=None, round_type=None):
    q = Round.query
    if user_id:
...
def sync_score(email: str, data: dict):
    """
    Submit a round scorecard, compute WHS differential, store Round, update user handicap_index.
    ...
    all_diffs = [
        ...
        for r in Round.query.filter_by(user_id=user.id)
